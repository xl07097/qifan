            ...
            $.ajax({
                ...
                success:function (data) {
                    ...
                    cook.forEach(function (item) {
                        var divList = $('<div class="clist"></div>');

                        var divImg = $('<div class="himg"></div>');
                        divlist.append(divImg);
                        var iconA = $(' <a href="/video/detail?id=<%= item.video_id %>">' +
                            '<img src="/images/<%= item.video_img %>" alt="<%= item.video_img %>" title="<%= item.video_img %>"></a>');
                        divImg.append(iconA);

                        var divInfo = $('<div class="videoinfo"></div>');
                        var pA = $(' <p>' +
                            '<a href="/video/detail?id=<%= item.video_id %>"><%= item.video_name %></a></p>');
                        var pSpan = $('<p class=""><a href="/personals/?id=<%= item.t_user_user_id %>"><%= item.user_name %></a>' +
                            '<span class="fr"><span>收藏<%= item.video_collects %></span>&nbsp;' +
                            '<span>点赞<%= item.video_likes %></span></span></p>');
                        divInfo.append(pA,pSpan);
                        divList.append(divInfo);
                        $(".showList").append(divList);
                    });
